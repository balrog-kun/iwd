...
/*
 * Called by both backends when netconfig_commit finishes, synchronously or
 * asynchronously.
 */
					...
					netconfig_build_entry_fn build_entry)
{
	...
	for (; value; value = value->next) {
		...
		build_entry(builder, value->data);
					...
					const void *data)
{
	...
	uint8_t family = l_rtnl_address_get_family(addr);
						...
						const void *data)
{
	...
	/* TODO: byte Preference = ICMPv6 preference */
					...
					enum l_netconfig_event event)
{
	...
	netconfig_agent_append_dict_strv(builder, "DomainNames", domains);
